function GraphicManager() {
    ...
    // =============================================================
    // QR CODE (MODERNO E LEGGIBILE)
    // =============================================================
    this.popolaQRCode = function () {
        $("#qrcode").empty();
        
        try {
            // ✅ RECUPERA IL TESTO PULITO (ORDINE: Nome... Tavolo... Elenco...)
            var testoOrdine = qrcodeManager.generaTestoOrdine();

            // ✅ GENERA IL QR STILIZZATO (Verde e Arrotondato)
            ...
            console.error("Errore generazione QR. Assicurati che QRCodeStyling sia caricato nell'index.", e);
            // Fallback se la libreria non carica (opzionale)
            $("#qrcode").html("<p>Errore nella generazione del QR Code</p>");
        }
    };
